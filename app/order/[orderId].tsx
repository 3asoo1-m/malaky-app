...
import { MenuItemImage, OptionGroup } from '@/lib/types'; // استيراد أنواع إضافية
...
interface OrderDetails {
    ...
    order_items: {
        ...
        menu_items: {
            ...
            // ✅ 1. تحديث نوع الصور ليكون مصفوفة
...
// --- مكون شريط تتبع الحالة (Status Tracker) ---
const StatusTracker = ({ currentStatus, orderType }: { currentStatus: string; orderType: 'delivery' | 'pickup' }) => {
    // تحديد الحالات بناءً على نوع الطلب
    const deliveryStatuses = ['new', 'processing', 'ready', 'delivered'];
    const pickupStatuses = ['new', 'processing', 'ready', 'collected'];
    const statuses = orderType === 'delivery' ? deliveryStatuses : pickupStatuses;
...
export default function OrderDetailsScreen() {
    ...
    useEffect(() => {
        const fetchOrderDetails = async () => {
            ...
            setLoading(true);
            try {
                // ✅ 2. تحديث الاستعلام لجلب الصور من الجدول الجديد
    ...
    const renderAddress = () => (
        <View style={styles.card}>
            <View style={styles.cardHeader}>
                ...
                <Text style={styles.cardTitle}>{order.order_type === 'delivery' ? 'عنوان التوصيل' : 'فرع الاستلام'}</Text>
            </View>
        ...
        <View style={styles.card}>
            ...
            <View style={styles.cardContent}>
                {order.order_items.map((item, index) => {
                    if (!item.menu_items) return null; // ✅ 3. إضافة تحقق للأمان
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            {/* ✅ 5. جعل ScrollView يأخذ المساحة المتبقية */}
...
const styles = StyleSheet.create({
    ...
    header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingBottom: 12, backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
    headerTitle: { fontSize: 20, fontFamily: 'Cairo-Bold', color: '#1A1A1A' },
    scrollHeader: { paddingVertical: 20, alignItems: 'center' },
    scrollHeaderTitle: { fontSize: 28, fontFamily: 'Cairo-Bold', color: '#1F2937' },
    scrollHeaderSubtitle: { fontSize: 16, fontFamily: 'Cairo-Regular', color: '#6B7280', marginTop: 4 },
    ...
    cardHeader: { alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' },
    cardTitle: { fontSize: 18, fontFamily: 'Cairo-Bold', color: '#1F2937', marginStart: 8 },
    cardContent: { padding: 16, alignItems: 'center' },
    infoTextBold: { fontSize: 16, fontFamily: 'Cairo-SemiBold', color: '#374151', textAlign: 'right' },
    infoText: { fontSize: 14, fontFamily: 'Cairo-Regular', color: '#6B7280', marginTop: 4, textAlign: 'right' },
    ...
    itemDetails: { flex: 1, marginStart: 12, alignItems: 'center' },
    itemName: { fontSize: 16, fontFamily: 'Cairo-SemiBold', color: '#374151' },
    optionsText: { fontSize: 13, fontFamily: 'Cairo-Regular', color: '#6B7280', marginTop: 2, textAlign: 'right' },
    notesText: { fontSize: 13, fontFamily: 'Cairo-Regular', color: '#9CA3AF', fontStyle: 'italic', marginTop: 2, textAlign: 'right' },
