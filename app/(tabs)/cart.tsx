...
import { useCart, CartItem, OrderType, Address } from '@/lib/useCart'; // 1. استيراد واجهة Address من useCart
...
export default function CartScreen() {
  ...
  // 2. استخدام كل الحالات المطلوبة من السياق العام useCart
  const {
    items,
    updateQuantity,
    subtotal,
  ...
  // 3. حالة محلية فقط لتخزين قائمة العناوين المتاحة
  const [availableAddresses, setAvailableAddresses] = useState<Address[]>([]);
  const [loadingAddresses, setLoadingAddresses] = useState(false);

  // 4. دالة جلب العناوين، معدلة لتستخدم الحالات الصحيحة
  const fetchAddresses = useCallback(async () => {
    ...
    if (error) {
      ...
      setAvailableAddresses(formattedData); // تحديث قائمة العناوين المتاحة

      if (formattedData.length > 0) {
        // إذا لم يكن هناك عنوان محدد في السياق العام، حدد الأول كافتراضي
        if (!selectedAddress) {
          setSelectedAddress(formattedData[0]);
        }
      } else {
        // إذا لم تعد هناك عناوين، امسح العنوان المختار من السياق العام
  ...
  // 5. استخدام useFocusEffect لجلب العناوين عند العودة إلى الشاشة
  useFocusEffect(
    useCallback(() => {
  ...
  // 6. useEffect لتحديث سعر التوصيل بناءً على العنوان المختار من السياق
  useEffect(() => {
  ...
  const AddressSection = () => {
    ...
    // 7. التحقق من قائمة العناوين المتاحة
        ...
        <TouchableOpacity style={styles.selectedAddress} onPress={() => router.push({ pathname: '/addresses', params: { fromCart: 'true' } })}>
          ...
          <View style={styles.addressTextContainer}>
            ...
            <Text style={styles.addressCity}>
              {selectedAddress ? `${selectedAddress.delivery_zones?.city}, ${selectedAddress.street_address}` : ''}
            </Text>
          </View>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <FlatList
        ...
        contentContainerStyle={[styles.listContainer, { paddingBottom: insets.bottom + 200 }]}
        ListHeaderComponent={
          <>
...
const styles = StyleSheet.create({
  ...
  quantityButton: { width: 30, height: 30, borderRadius: 15, backgroundColor: '#FEECEB', justifyContent: 'center', alignItems: 'center' },
  quantityText: { fontSize: 16, fontWeight: 'bold' },
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#fff', padding: 16, borderTopWidth: 1, borderTopColor: '#eee', elevation: 10 },
